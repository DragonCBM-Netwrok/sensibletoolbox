    ...
    public void setBlockProtectionType(BlockProtectionType blockProtectionType) {
        switch (blockProtectionType) {
            case BEST:
                ...
                } else if (SensibleToolbox.getPluginInstance().isGriefPreventionAvailable()) {
                    this.blockProtectionType = BlockProtectionType.GRIEFPREVENTION;
                } else {
    ...
    public boolean playerCanBuild(Player player, Block block, Operation op) {
        switch (blockProtectionType) {
            ...
            case GRIEFPREVENTION:
                if (GriefPrevention.instance.allowBuild(player, block.getLocation(), block.getType()) != null)
                {
                    return false;
                }
                else
    ...
    public enum BlockProtectionType {
        ...
        public boolean isAvailable() {
            switch (this) {
                ...
                case GRIEFPREVENTION: return SensibleToolbox.getPluginInstance().isGriefPreventionAvailable();
                default: return true;
            }
        }
    }
