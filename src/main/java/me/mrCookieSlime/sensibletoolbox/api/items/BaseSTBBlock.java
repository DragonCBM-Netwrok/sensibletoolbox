...
public abstract class BaseSTBBlock extends BaseSTBItem {
    ...
    protected BaseSTBBlock(ConfigurationSection conf) {
        ...
        accessControl = AccessControl.valueOf(conf.getString("accessControl", "RESTRICTED"));
        ticksLived = 0;
...
//    }
    ...
    public final void placeBlock(final Block block, Player player, BlockFace facing) {
        placeBlock(block,player.getUniqueId(),facing);
    }
    public final void placeBlock(final Block block, UUID player, BlockFace facing) {
        placeBlock(block,player,facing, block.getLocation());
    }

    public final void placeBlock(final Block block, UUID player, BlockFace facing, Location location) {
    ...
    public final void breakBlock(boolean dropItem) {
        ...
        if (dropItem) {
            ...
            //BaseSTBItem item = SensibleToolbox.getItemRegistry().getItemById(this.getItemTypeID());
        ...
        if (block.getLocation().equals(this.getLocation())) {
            if (getSkullData() != null) {
                try {
                    ...
                    CustomSkull.setSkull(block, getSkullOwner());
                } catch (Exception e) {
