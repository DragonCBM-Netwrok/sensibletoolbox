...
public abstract class BaseSTBItem implements Comparable<BaseSTBItem>, InventoryGUIListener {
    ...
    protected final void registerCustomIngredients(BaseSTBItem... items) {
        for (BaseSTBItem item : items) {
                SensibleToolboxPlugin.getInstance().getItemRegistry().addCraftingRestriction(this, SensibleToolboxPlugin.getSTBItemMaterialData(item).getItemType(), item.getClass());
        }
    }
    /**
    ...
    public final boolean isIngredientFor(ItemStack result) {
        ...
        Class<? extends BaseSTBItem> c = item.getCraftingRestriction(SensibleToolboxPlugin.getSTBItemMaterialData(this).getItemType());
        return c == getClass();
    ...
    {
        if (skulltexture.equals("") || skulltexture == null)
        {
            return null;
        }
        try {
            return CustomSkull.getItem(skulltexture);
        }
        catch (Exception e)
        {
            return null;
